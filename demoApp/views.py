...
def status():
    default_data['status'] = Status.objects.all()
...
def edit_item(request, pk):
    ...
    if request.POST:
        ...
        master.Email = request.POST['email']
        master.Password = request.POST['password']
        master.Status = Status.objects.get(Status = request.POST['status'])
        master.save()
    else:
...
def mulDelete(request):
    print('before: ',request.POST)
    val = request.POST['selectDelete']
    print(val)
    # val = int(val)
    # print('after: ',val)
    # b = []
    # b.append(val)
    # c = b
    # Master.objects.get(id=val).delete()
    return redirect(index)
